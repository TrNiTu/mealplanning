...
const InventoryList = ({ account }) => {
  ...
  const [addItemNameValidity, setAddItemNameValidity] = useState("");
  ...
  const handleAddItemConfirm = async (item) => {
    setAddItemCategoryValidity(CATEGORIES.includes(item.category));
    setAddItemNameValidity(item.name ? true : false);

    // IF BOTH NAME AND CATEGORY VALIDITY ARE TRUE, CAN CLOSE MODAL
    if (addItemCategoryValidity && addItemNameValidity) {
      setIsLoading(true);
      ...
      {isLoading ? (
        ...
        <Flex direction="column">
          ...
          <Flex width="75vw" maxWidth="75vw">
            ...
            />
            <AddItemModal
              categoryValidity={addItemCategoryValidity}
              nameValidity={addItemNameValidity}
